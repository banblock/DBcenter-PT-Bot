...
# 로봇별 도킹 스테이션 앞 대기(진입) 지점. 정밀한 도킹 자세가 아니라
# "Dock 액션(도킹 스테이션 IR 비콘 기반 자동 접근)이 알아서 붙을 수
# 있는 근방"이면 된다. x/y는 실측값. yaw는 여기서 고정하지 않는다 -
# fleet_node._on_dock_return()이 진입점 도착 후 실제 도크(= 로봇의 초기
# 위치 DEFAULT_ROBOT_START)를 바라보는 각도로 최종 yaw를 자동 계산한다.
# 아래 yaw 값은 그 초기 위치가 없는 로봇에 대한 폴백으로만 남겨둔다.
DOCK_STATIONS = {
